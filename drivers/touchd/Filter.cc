...
void FlipFilter::processMouseButton( int button, int state, int x, int y )
{
#ifdef HAS_FREENECT
	if(useIntensityImage) 
	{
		IntensityImage* inputimg = input->getImage();
	std::cout << "Pos = " << x << ", " << y << " -- Depth = " << inputimg->getPixel(x,y) << std::endl;
	}
	else
	{
		ShortImage* inputimg = input->getShortImage();
	std::cout << "Pos = " << x << ", " << y << " -- Depth = " << inputimg->getPixel(x,y)/32 << std::endl;
	}
...
ThreshFilter::ThreshFilter( TiXmlElement* _config, Filter* _input ): Filter( _config, _input ) {
	...
	config->QueryIntAttribute(  "ForwardValues", &forward_values );
...
void ThreshFilter::modifyOptionValue(double delta, bool overwrite) {
	...
	case 2:
		if(overwrite) {
			forward_values = (delta < 0) ? 0 : (delta > 1) ? 1 : delta;
		} else {
			forward_values += delta;
			forward_values = (forward_values < 0) ? 0 : (forward_values > 1) ? 1 : forward_values;
		}
		break;
	}
}
...
TiXmlElement* ThreshFilter::getXMLRepresentation() {
	...
	XMLNode->SetAttribute( "ForwardValues" , forward_values);
...
AquaTopBGGenFilter::AquaTopBGGenFilter( TiXmlElement* _config, Filter* _input ): Filter( _config, _input ) {
	...
	config->QueryIntAttribute( "Invert",   &invert   );
	config->QueryIntAttribute( "Adaptive", &adaptive );
	bggenerator = new AquaTopBGGenerator();
	// setting variables for Configurator
	countOfOptions = 2; // quantity of variables that can be manipulated
}
...
void AquaTopBGGenFilter::link( Filter* _mask ) {
	mask = _mask;
}
